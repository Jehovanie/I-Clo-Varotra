function AfterNavbar() {
	const listCategory = [1, 2, 3, 4, 5, 6, 7].map((item, index) => {
		return (
			<option key={index + "_option_category"} className="text-lg font-thin">
				{" "}
				Category {index + 1}{" "}
			</option>
		);
	...
	const listLink_Html = listLink.map((item, index) => {
		return (
			<li key={index + "_listLink"}>
				{" "}
				<a href={item.link} className="font-montserrat leading-normal font-normal text-gray-500 text-base">
					{item.name}
				</a>
			</li>
		);
		...
		<div className="px-5 py-3 z-10 w-full bg-gray-200 ">
			<nav className="flex justify-between items-center max-container">
				...
				<div className="text-lg leading-normal font-montserrat max-lg:hidden w-3/5">
					<ul className="flex gap-2 justify-start items-center max-containe gap-4">{listLink_Html}</ul>
				</div>
